...
def send():
    ...
    if lat is None or lon is None:
        ...
        sql = text('INSERT INTO simple_tracking.positions (devices_id, data_ts, position, data) VALUES (get_device_id(:ident), TO_TIMESTAMP(:data_ts), ST_SetSRID(st_point(:lon,:lat), 4326), :data) RETURNING id')
        with  db.engine.connect() as connection:
            result = connection.execute(sql, {
                'ident': ident, 
                'data_ts': ts,
            ...
            connection.commit()
            request_data['insert_id'] = result.fetchone().id
...
def send_ttn():
    ...
    if lat is None or lon is None:
        ...
        sql = text('INSERT INTO simple_tracking.positions (devices_id, data_ts, position, data) VALUES (get_device_id(:ident), (:data_ts)::timestamp, ST_SetSRID(st_point(:lon,:lat), 4326), :data) RETURNING id')
        with db.engine.connect() as connection:
            result = connection.execute(sql, {
                'ident': ident, 
                'data_ts': ts,
